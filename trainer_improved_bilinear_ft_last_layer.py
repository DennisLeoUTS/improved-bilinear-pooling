...
class NetworkManager(object):
    def __init__(self, options, path):
        ...
        if stage == "train_all":
            self.net.load_state_dict(torch.load('./models/saved/VGG/VGG_bilinear_norm_train_last_layer.pkl'))
            1
        else:
        ...
        if stage == "train_all":
            self.schedule = torch.optim.lr_scheduler.ReduceLROnPlateau(
                ...
                threshold=1e-4)
            # self.schedule = torch.optim.lr_scheduler.StepLR(self.solver, step_size=50, gamma=0.1)
    ...
    def train(self):
        ...
        for epoch in range(self.options['epochs']):
            ...
            if test_acc_epoch>best_acc:
                ...
                best_epoch = epoch+1
                torch.save(self.net.state_dict(), os.path.join(self.path['model_save'], self.options['net_choice'],
                                                               "VGG_bilinear_norm_"+stage+'.pkl'))
